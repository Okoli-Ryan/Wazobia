    ...
    <View
      ...
      <View
        ...
        <TouchableOpacity onPress={() => setBookmark((prev) => !prev)}>
          <Image
            source={bookmark ? BookmarkDone : Bookmark}
...
const SubHeader = (screen, dispatch) => {
  if (screen === 1) return <CategoryHeader dispatch={dispatch} />;
  else if (screen === 2) return <SearchHeader dispatch={dispatch} />;
  else if (screen === 3) return <TopicHeader />;
  else if (screen === 4) return <InfoHeader />;
  else return null;
};
